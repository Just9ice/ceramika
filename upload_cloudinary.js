...
async function uploadImage(localPath, publicId) {
  if (!fs.existsSync(localPath)) {
    console.log(`Skipping: File not found - ${localPath}`);
    return;
  }
  ...
  try {
    ...
    const result = await cloudinary.uploader.upload(localPath, {
      public_id: publicId,
      overwrite: true,
      folder: "products" // Organize them in a products folder
    });
...
async function run() {
  ...
  for (const [sku, files] of Object.entries(skuMappings)) {
    if (files.main) {
      await uploadImage(path.join(publicDir, files.main), `${sku}_main`);
      count++;
    }
    if (files.alt) {
      await uploadImage(path.join(publicDir, files.alt), `${sku}_alt`);
      count++;
    }
    if (files.env) {
      await uploadImage(path.join(publicDir, files.env), `${sku}_env`);
      count++;
    }
  }
  
  console.log(`\nFinished processing! Attempted to upload ${count} images.`);
}
