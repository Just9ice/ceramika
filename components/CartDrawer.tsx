    ...
    <div className='fixed inset-0 z-[100] flex justify-end'>
      ...
      >
        ...
        <div className="flex flex-shrink-0 items-center justify-between px-6 py-5 border-b border-border/50 bg-card">
          <h2
            className="text-card-foreground font-serif text-2xl font-black tracking-tight"
          >
              ...
              >
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-3 py-2.5 bg-input border border-border rounded-xl text-base sm:text-sm outline-none focus:border-accent text-foreground transition-colors placeholder-muted-foreground" placeholder="John Doe" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-foreground/70 uppercase tracking-widest mb-1.5">Phone Number *</label>
                    <input required type="tel" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full px-3 py-2.5 bg-input border border-border rounded-xl text-base sm:text-sm outline-none focus:border-accent text-foreground transition-colors placeholder-muted-foreground" placeholder="08012345678" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-foreground/70 uppercase tracking-widest mb-1.5">Email Address</label>
                    <input type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full px-3 py-2.5 bg-input border border-border rounded-xl text-base sm:text-sm outline-none focus:border-accent text-foreground transition-colors placeholder-muted-foreground" placeholder="john@example.com" />
                  </div>
                  <div>
                    ...
                    <textarea value={formData.homeAddress} onChange={e => setFormData({...formData, homeAddress: e.target.value})} rows={2} className="w-full px-3 py-2.5 bg-input border border-border rounded-xl text-base sm:text-sm outline-none focus:border-accent text-foreground transition-colors placeholder-muted-foreground resize-none" placeholder="123 Ceramic Way..." />
                  </div>
                  <div>
                    ...
                    <textarea value={formData.officeAddress} onChange={e => setFormData({...formData, officeAddress: e.target.value})} rows={2} className="w-full px-3 py-2.5 bg-input border border-border rounded-xl text-base sm:text-sm outline-none focus:border-accent text-foreground transition-colors placeholder-muted-foreground resize-none" placeholder="Corporate HQ..." />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-foreground/70 uppercase tracking-widest mb-1.5">Dispatch Location *</label>
                    <input required type="text" value={formData.dispatchLocation} onChange={e => setFormData({...formData, dispatchLocation: e.target.value})} className="w-full px-3 py-2.5 bg-input border border-border rounded-xl text-base sm:text-sm outline-none focus:border-accent text-foreground transition-colors placeholder-muted-foreground" placeholder="Which address should we deliver to?" />
                  </div>
                </div>
              </motion.form>
            )}
        ...
        {items.length > 0 && (
          <div className="flex-shrink-0 px-6 py-5 border-t border-border/50 bg-card flex flex-col gap-4 shadow-[0_-10px_40px_rgba(0,0,0,0.03)]">
            <div className="flex items-center justify-between">
              ...
              <span className="font-serif font-black text-2xl text-foreground">
