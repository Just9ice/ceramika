        ...
        <div className="min-h-screen bg-background overflow-x-hidden">
            ...
            <section className="relative pt-36 pb-16 overflow-hidden border-b border-border">
                ...
                <div
                    ...
                    style={{ background: 'radial-gradient(ellipse 60% 50% at 50% -5%, rgba(200,169,110,0.09) 0%, transparent 70%)' }}
                />
                {/* Grid */}
                <div
                ...
                {/* Diagonal accent */}
                <div
                    className="absolute top-0 right-16 w-px h-full opacity-8 pointer-events-none hidden lg:block"
                ...
                <div className="relative max-w-5xl mx-auto px-4 sm:px-6">
                    ...
                    <motion.h1
                        ...
                        style={{ fontFamily: "'Georgia', serif", fontSize: 'clamp(2.2rem, 5vw, 3.8rem)' }}
                    ...
                    <motion.p
                        className="text-muted-foreground text-base max-w-2xl leading-relaxed mb-6"
                    ...
                    >
                        <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground bg-muted border border-border rounded-full px-3 py-1.5">
                            <span className="text-[#a68038] dark:text-[#c8a96e]/50">Effective</span>
                            <span className="text-foreground/70 font-medium">{effectiveDate}</span>
                        </span>
                        <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground bg-muted border border-border rounded-full px-3 py-1.5">
                            <span className="text-[#a68038] dark:text-[#c8a96e]/50">Updated</span>
                        ...
                        <nav className="flex flex-col gap-0.5">
                            {sections.map((s) => (
                                <a
                                    key={s.id}
                                    href={`#${s.id}`}
                                    className="group flex items-center gap-2 text-muted-foreground hover:text-[#a68038] dark:text-[#c8a96e] text-xs py-2 pl-3 border-l border-border hover:border-[#a68038] dark:border-[#c8a96e]/40 transition-all duration-200"
                                >
                    ...
                    <div className="flex flex-wrap gap-3">
                        {[
                            ...
                            <a
                                key={link.href}
                                href={link.href}
                                className="text-sm text-muted-foreground hover:text-[#a68038] dark:text-[#c8a96e] border border-border hover:border-[#a68038] dark:border-[#c8a96e]/30 rounded-full px-4 py-2 transition-all duration-200 hover:-translate-y-0.5"
                            >
                                {link.label}
                            </a>
                        ))}
